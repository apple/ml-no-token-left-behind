...
from external.StyleCLIP.criteria.clip_loss import CLIPLoss, EGCLIPLoss
from external.StyleCLIP.criteria.id_loss import IDLoss
from external.StyleCLIP.mapper.training.train_utils import STYLESPACE_DIMENSIONS
...
from external.StyleCLIP.utils import ensure_checkpoint_exists
...
def main(args):
    ...
    assert len(args.desired_words.split(' ')) == len(args.description.split(' '))
    ...
    if args.loss_type == "EGCLIPLoss":
        clip_loss = EGCLIPLoss(args)
    elif args.loss_type == "EGCLIPLossWithDropout":
        clip_loss = EGCLIPLossWithDropout(args)
    elif args.loss_type == "EGCLIPLoss_New":
        clip_loss = EGCLIPLoss_New(args)
    else:
        raise Exception("The loss type specified does not exist")
    ...
    return final_result, img_orig, img_gen
...
if __name__ == "__main__":
    ...
    parser.add_argument('--desired_words', default=None, type=str,
                             help="Focus mwsure for each word in the description, a float fot each word, seperated by space")
    ...
    parser.add_argument('--expl_lambda', default=1., type=float,
                             help="Weight of explainability loss")
    parser.add_argument("--loss_type", type=str, choices=["EGCLIPLossWithDropout", "EGCLIPLoss", "EGCLIPLoss_New"], default="EGCLIPLoss")
