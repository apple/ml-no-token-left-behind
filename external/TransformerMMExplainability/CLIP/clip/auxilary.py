...
class MultiheadAttention(torch.nn.Module):
    ...
    def multi_head_attention_forward(self, query, key, value, key_padding_mask=None,
                need_weights=True, attn_mask=None, use_separate_proj_weight=False,
                attention_probs_forward_hook=None, attention_probs_backwards_hook=None, **kwargs):
        if not torch.jit.is_scripting():
            tens_ops = (query, key, value, self.in_proj_weight, self.in_proj_bias, self.bias_k, self.bias_v,
                        self.out_proj.weight, self.out_proj.bias)
            if any([type(t) is not Tensor for t in tens_ops]) and F.has_torch_function(tens_ops):
                return F.handle_torch_function(
                    self.multi_head_attention_forward, tens_ops, query, key, value, key_padding_mask, need_weights,
                    attn_mask, attention_probs_forward_hook=attention_probs_forward_hook, 
        ...
        if self.bias_k is not None and self.bias_v is not None:
            if self.static_k is None and self.static_v is None:
        ...
        if self.static_k is not None:
            assert self.static_k.size(0) == bsz * self.num_heads
            assert self.static_k.size(2) == head_dim
            k = self.static_k

        if self.static_v is not None:
            assert self.static_v.size(0) == bsz * num_heads
            assert self.static_v.size(2) == head_dim
            v = self.static_v
        ...
        self.attn_output_weights = attn_output_weights # My change
        attn_output_weights = F.dropout(attn_output_weights, p=self.dropout, training=self.training)
        attn_output_weights = torch.nn.Identity()(attn_output_weights)
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return self.multi_head_attention_forward(query, key, value, key_padding_mask, need_weights, attn_mask, use_separate_proj_weight=True, 
                                                     attention_probs_forward_hook=attention_probs_forward_hook, attention_probs_backwards_hook=attention_probs_backwards_hook)
        else:
            return self.multi_head_attention_forward(query, key, value, key_padding_mask, need_weights, attn_mask, 
                                                     attention_probs_forward_hook=attention_probs_forward_hook, attention_probs_backwards_hook=attention_probs_backwards_hook)
